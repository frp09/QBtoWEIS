...
class Controller():
    ...
    def __init__(self, controller_params):
        ...
        # Minimum pitch saturation parameters
        self.U_ps = list_check(controller_params['U_ps'], return_bool=False)
        self.ps_angles_pc = list_check(controller_params['ps_angles_pc'], return_bool=False)
        ...
        # Error checking: pitch gain schedule inputs  #FP
        if list_check(self.U_ps) and \
            (list_check(self.ps_angles_pc) and \
                not len(self.U_ps) == len(self.ps_angles_pc)):
            raise Exception(
                'U_ps, ps_angles_pc are all list-like and are not of equal length')
    ...
    def tune_controller(self, turbine):
        ...
        # Resample ps_angles_pc and overwrite previous choice (PS_Mode) #FP
        if not list_check(self.ps_angles_pc):
            self.ps_angles_pc_U = self.ps_angles_pc * np.ones(len(self.v))
            self.ps_min_bld_pitch = self.ps_angles_pc_U  #FP overwrite previous generic tuning
        else: 
            if self.interp_type == 'sigma':
                self.ps_angles_pc_U = multi_sigma(self.v, self.U_ps, self.ps_angles_pc)
                self.ps_min_bld_pitch = self.ps_angles_pc_U  #FP overwrite previous generic tuning
            else: 
                interp_ps_angles_pc_U = interpolate.interp1d(self.U_ps, self.ps_angles_pc, kind=self.interp_type, bounds_error=False, fill_value='extrapolate')
                self.ps_angles_pc_U = interp_ps_angles_pc_U(self.v)
                self.ps_min_bld_pitch = self.ps_angles_pc_U  #FP overwrite previous generic tuning
